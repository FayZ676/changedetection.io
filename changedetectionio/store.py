...
class ChangeDetectionStore:
    ...
    def __init__(
        ...
        self.generic_definition = deepcopy(
            Watch.model(datastore_path=datastore_path, default={})
        )
        ...
        # First time ran, Create the datastore.
            ...
            if include_default_watches:
                print(
                    "No JSON DB found at {}, creating JSON store at {}".format(
    ...
    def set_last_viewed(self, uuid, timestamp):
        logging.debug(
            "Setting watch UUID: {} last viewed to {}".format(uuid, int(timestamp))
        )
    ...
    def add_watch(
        self, url, tag="", extras=None, tag_uuids=None, write_to_disk_now=True
    ):
        if extras is None:
            extras = {}
        ...
        apply_extras["tags"] = (
            [] if not apply_extras.get("tags") else apply_extras.get("tags")
        )
    ...
    def sync_to_json(self):
        ...
        try:
            ...
            try:
                ...
                logging.error(
                    "Error writing JSON!! (Main JSON file save was skipped) : %s",
                    str(e),
                )
    ...
    def get_preferred_proxy_for_watch(self, uuid):
        ...
        if (
            strtobool(os.getenv("ENABLE_NO_PROXY_OPTION", "True"))
            and watch.get("proxy") == "no-proxy"
        ):
            return None
    ...
    def get_all_headers_in_textfile_for_watch(self, uuid):
        ...
        if watch:
            ...
            for tag_uuid, tag in tags.items():
                fname = (
                    "headers-"
                    + re.sub(r"[\W_]", "", tag.get("title")).lower().strip()
                    + ".txt"
                )
    ...
    def add_tag(self, name):
        ...
        with self.lock:
            new_tag = Watch.model(
                ...
                default={"title": name.strip(), "date_created": int(time.time())},
            )
    ...
    def get_all_tags_for_watch(self, uuid):
        ...
        if watch:
            return dictfilt(
                self.__data["settings"]["application"]["tags"], watch.get("tags", [])
            )
    ...
    def tag_exists_by_name(self, tag_name):
        return any(
            v.get("title", "").lower() == tag_name.lower()
            for k, v in self.__data["settings"]["application"]["tags"].items()
        )
    ...
    def update_1(self):
        if self.data["settings"]["requests"].get("minutes_between_check"):
            self.data["settings"]["requests"]["time_between_check"][
                "minutes"
            ] = self.data["settings"]["requests"]["minutes_between_check"]
        ...
        for uuid, watch in self.data["watching"].items():
            if "minutes_between_check" in watch:
                ...
                if watch.get("minutes_between_check", False):
                    self.data["watching"][uuid]["time_between_check"][
                        "minutes"
            ...
            if watch.get("history", False):
                ...
                if len(history):
                    ...
                    if os.path.exists(target_path):
                        ...
                        logging.warning(
                            "Datastore history directory {} does not exist, skipping history import.".format(
                                target_path
                            )
                        )
    ...
    def update_5(self):
        ...
        current_system_title = self.data["settings"]["application"][
            "notification_body"
        ].translate(str.maketrans("", "", "\r\n "))
        for uuid, watch in self.data["watching"].items():
            try:
                ...
                if (
                    watch_body
                    and watch_body.translate(str.maketrans("", "", "\r\n "))
                    == current_system_body
                ):
                ...
                if (
                    watch_title
                    and watch_title.translate(str.maketrans("", "", "\r\n "))
    ...
    def update_14(self):
        for awatch in self.__data["watching"]:
            if self.__data["watching"][awatch]["include_filters"]:
                for num, selector in enumerate(
                    self.__data["watching"][awatch]["include_filters"]
                ):
                    if selector.startswith("/"):
                        self.__data["watching"][awatch]["include_filters"][num] = (
                            "xpath1:" + selector
                        )
                    if selector.startswith("xpath:"):
                        self.__data["watching"][awatch]["include_filters"][
                            num
                        ] = selector.replace("xpath:", "xpath1:", 1)
